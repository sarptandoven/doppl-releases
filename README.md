...
No Doppl account, hosted workspace, or setup is required.
...
![Local-first](https://img.shields.io/badge/Local--first-Your%20work%20stays%20local-F7F8F5?labelColor=11120F)
...
- **Your work stays on machines you control.** Doppl does not relay room contents. Sharing a room
  connects your Macs directly over private HTTPS. Optional product analytics contain no prompts,
  answers, code, files, paths, room messages, or project names and can be disabled at any time;
  [PRIVACY.md](PRIVACY.md) documents the exact boundary.
...
Every supported build is signed with an Apple Developer ID, notarized by Apple, stapled, and
accepted by Gatekeeper. Each release ships checksums (`SHA256SUMS`) and build evidence, so you can
verify exactly what you're running. A release marked **Pre-release** is a controlled pilot, not the
stable channel.
...
4. To share a room, connect both Macs to the same Tailscale network. On the owner's Mac, open
   **Settings → Shared access**, enable the private HTTPS path, and create a one-use invitation.
...
- **No cloud relay for your work.** Doppl runs on your Mac and does not relay room contents.
